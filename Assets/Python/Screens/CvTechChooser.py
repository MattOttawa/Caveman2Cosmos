...
class CvTechChooser:
	...
	def screen(self): return CyGInterfaceScreen("TechChooser", self.screenId)
	...
	def cachePlayer(self, iPlayer):
		...
		self.iDisplayedEra = self.iCurrentEra = CyPlayer.getCurrentEra()

		self.currentTechState = aList0 = []
		self.eraTechBarred = aList1 = [0] * self.iNumEras
		self.eraTechAvailable = aList2 = [0] * self.iNumEras
		aList3 = []
		self.currentSelection = aList4 = [0, 1]
		iTech = 0
		while iTech < self.iNumTechs:
			if CyTeam.isHasTech(iTech):
				aList0.append(CIV_HAS_TECH)
			else:
				info = GC.getTechInfo(iTech)
				iEra = info.getEra()
				if not CyPlayer.canEverResearch(iTech):
					aList0.append(CIV_NO_RESEARCH)
					if not info.isDisable():
						aList1[iEra] = aList1[iEra] + 1
				else:
					aList2[iEra] = aList2[iEra] + 1
					if CyPlayer.canResearch(iTech):
						# Immidiate selection positions for its scroll-cycle functionality
						iX = info.getGridX()
						if iX not in aList3:
							aList3.append(iX)
					if not CyPlayer.isResearchingTech(iTech):
						aList0.append(CIV_TECH_AVAILABLE)
					else:
						queuePos = CyPlayer.getQueuePosition(iTech)
						if queuePos == 1:
							aList0.append(CIV_IS_RESEARCHING)
						elif queuePos == CyPlayer.getLengthResearchQueue():
							aList0.append(CIV_IS_TARGET)
						else:
							aList0.append(CIV_IS_QUEUED)
			iTech += 1
		aList3.sort()
		aList4[0] = len(aList3)
		while aList3:
			aList4.append(aList3.pop(0) * self.xCellDist - self.minX - self.xRes / 2 + self.xCellDist / 2)
	...
	def interfaceScreen(self):
		...
		for iTech in xrange(self.iNumTechs):
			...
			if iX > 0:
				...
				if iX < self.minX:
					self.minX = iX
		self.maxScroll = self.maxX - xRes - self.minX
		...
		screen.addPanel("TC_BarTop", "", "", True, False, 0, 0, xRes, hPnlBarTop, PanelStyles.PANEL_STYLE_TOPBAR)
		screen.addPanel("TC_BarBot", "", "", True, False, 0, yRes - hPnlBarBot, xRes, hPnlBarBot, PanelStyles.PANEL_STYLE_BOTTOMBAR)
		...
		if self.currentSelection[0]:
			screen.setButtonGFC("cycleTech0", "", "", 24, yRes - 30, 24, 24, eWidGen, 1, 2, ButtonStyles.BUTTON_STYLE_ARROW_LEFT)
			screen.setButtonGFC("cycleTech1", "", "", 56, yRes - 30, 24, 24, eWidGen, 1, 2, ButtonStyles.BUTTON_STYLE_ARROW_RIGHT)

		stackBar = "progressBar"
		screen.addStackedBarGFC(stackBar, 256, 2, xRes - 512, 32, InfoBarTypes.NUM_INFOBAR_TYPES, eWidGen, 1, 2)
		...
		screen.addPanel(SCREEN_PANEL, "", "", False, False, 0, hPnlBarTop - 6, self.maxX + self.xCellDist, yRes - hPnlBarTop - hPnlBarBot, PanelStyles.PANEL_STYLE_EMPTY)
		...
		screen.addPanel("TC_BarBotSlider", "", "", False, False, SLIDER_BORDER - 1, yRes - hPnlBarBot - 8, xRes, 28, PanelStyles.PANEL_STYLE_EMPTY)
		screen.setButtonGFC("scrollArrow0", "", "", -5, yRes - hPnlBarBot - 4, 22, 22, eWidGen, 1, 2, ButtonStyles.BUTTON_STYLE_ARROW_LEFT)
		screen.setButtonGFC("scrollArrow1", "", "", xRes - 18, yRes - hPnlBarBot - 4, 22, 22, eWidGen, 1, 2, ButtonStyles.BUTTON_STYLE_ARROW_RIGHT)

		# Displayed Era
		self.setDisplayedEra()

		# Jump to era
		screen.setText("TC_JumpTo", "", "<font=4b>" + TRNSLTR.getText("TXT_KEY_JUMP_TO", ()), 1<<1, xRes - 8, yRes - 32, 0, eFontTitle, eWidGen, 1, 2)
		table = "TC_EraTable"
		h = 24 * self.iNumEras + 2
		if h > yRes - 32:
			h = yRes - 32
		screen.setImageButton("TC_EraTableBG0", "", -8, -8, xRes + 16, yRes + 16, eWidGen, 1, 2)
		screen.addPanel("TC_EraTableBG", "", "", False, False, xRes - 274, yRes - h - 20, 280, h + 40, PanelStyles.PANEL_STYLE_SOLID)
		screen.addTableControlGFC(table, 1, xRes - 264, yRes - h - 8, 256, h, False, True, 30, 21, TableStyles.TABLE_STYLE_STANDARD)
		screen.setStyle(table, "Table_StandardCiv_Style")
		screen.setTableNumRows(table, self.iNumEras)
		screen.setTableColumnHeader(table, 0, "", 256)
		iEra = self.iNumEras - 1
		i = 0
		while iEra > -1:
			info = GC.getEraInfo(iEra)
			if iEra == self.iCurrentEra:
				szTxt = self.aFontList[4] + "<color=255,255,200>"
			else:
				szTxt = self.aFontList[5]
			szTxt += info.getDescription()
			if self.eraTechAvailable[iEra] or self.eraTechBarred[iEra]:
				if self.eraTechAvailable[iEra] and self.eraTechBarred[iEra]:
					szTxt += " &#60--&#62 %s<color=100,255,200>%d</color>, <color=255,30,30>%d" %(self.aFontList[4], self.eraTechAvailable[iEra], self.eraTechBarred[iEra])
				elif self.eraTechAvailable[iEra]:
					szTxt += " &#60--&#62 " + self.aFontList[4] + "<color=100,255,200>" + str(self.eraTechAvailable[iEra])
				else:
					szTxt += " &#60--&#62 " + self.aFontList[4] + "<color=255,30,30>" + str(self.eraTechBarred[iEra])
			screen.setTableText(table, 0, i, szTxt, info.getButton(), eWidGen, 1, 2, 1<<0)
			iEra -= 1
			i += 1
		self.hideEraTable(screen)

		# On first load we will scroll to the currently researched tech automatically
		self.initialScroll()

	def initialScroll(self):
		pos = self.getLastResearchingIdx()
		if pos < 0:
			pos = self.getLastResearchedIdx()

		if pos != -1:
			self.scrollToTech(pos)
		else:
			self.scrollTo(self.scrollOffs)

	def getTechPos(self, idx):
		return GC.getTechInfo(idx).getGridX() * self.xCellDist - self.minX
	...
	def setDisplayedEra(self):
		screen = self.screen()
		info = GC.getEraInfo(self.iDisplayedEra)
		img = info.getButton()
		if img:
			screen.setText("WID|ERA0", "", "<img=%s>" % img, 1<<0, self.xRes/2 + 64, self.yRes - 32, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, 1, 2)
			screen.setText("WID|ERA1", "", "<font=4b>" + info.getDescription(), 1<<1, self.xRes/2 + 60, self.yRes - 32, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, 1, 2)
		else:
			screen.setText("WID|ERA0", "", "<font=4b>" + info.getDescription(), 1<<2, self.xRes/2, self.yRes - 32, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, 1, 2)

	def scroll(self):
		self.screen().moveItem(SCREEN_PANEL, -self.scrollOffs, hPnlBarTop - 6, 0)

		# Special edge case
		if self.scrollOffs > self.maxScroll - self.wCell:
			if self.iDisplayedEra != self.iNumEras - 1:
				self.iDisplayedEra = self.iNumEras - 1
				self.setDisplayedEra()
		# General case
		elif self.scrollOffs < self.minEraX[self.iDisplayedEra] - self.minX - self.wCell or self.scrollOffs >= self.minEraX[self.iDisplayedEra + 1] - self.minX - self.wCell:
			self.iDisplayedEra = iEra = 0
			while iEra < self.iNumEras:
				if self.scrollOffs < self.minEraX[iEra] - self.minX - self.wCell:
					break
				self.iDisplayedEra = iEra
				iEra += 1
			self.setDisplayedEra()
	...
	def scrollTo(self, offs):
		...
		if self.scrollOffs < 0:
			self.scrollOffs = 0
		if self.scrollOffs > self.maxScroll:
			self.scrollOffs = self.maxScroll
		self.screen().attachSlider("TC_BarBotSlider", "TC_Slider", 0, 0, self.xRes - SLIDER_BORDER * 2 - 11, 0, self.scrollOffs, 0, self.maxScroll, WidgetTypes.WIDGET_GENERAL, 0, 0, False)
		self.scroll()
	...
	def refresh(self, techs, bFull):
		...
		dy = self.yRes - hPnlBarTop - hPnlBarBot

		yEmptySpace = (dy - self.iVerticalCells * self.hCell) / self.iVerticalCells
		if yEmptySpace < 0:
			yEmptySpace = 0
		...
		for iTech in techs:
			...
			if not bFull:
				iX = x0 * self.xCellDist - self.minX + 12
				...
				for iTechX in CvTechInfo.getPrereqOrTechs():
					...
					iX = x1 * self.xCellDist + self.wCell - self.minX + 12
					iY = yEmptySpace + ((y1 - 1) * yCellDist) / 2 - 2

					xDiff = x0 - x1
					yDiff = y0 - y1
					xOff = xDiff * CELL_GAP + (xDiff - 1) * self.wCell - 4

					# Helper functions for drawing the tech dependency arrows
					def add_arrow_head(x, y):
						screen.addDDSGFCAt("", SCREEN_PANEL, ARROW_HEAD, x, y, 8, 8, eWidGen, 1, 2, False)

					def add_line_h(x, y, len):
						screen.addDDSGFCAt("", SCREEN_PANEL, ARROW_X, x, y, len, 8, eWidGen, 1, 2, False)

					def add_line_v(x, y, len):
						screen.addDDSGFCAt("", SCREEN_PANEL, ARROW_Y, x, y, 8, len, eWidGen, 1, 2, False)
	...
	def updateTechRecords(self, bForce):
		...
		if lChanged:
			...
			for iTech, CvTechInfo, iEra in lChanged:
				...
				screen.setLabelAt(TECH_NAME + szTech, szTechRecord, szTechString, 1<<0, iX, iY, 0, eFontTitle, eWidGen, 1, 2)
				screen.setHitTest(TECH_NAME + szTech, HitTestTypes.HITTEST_NOHIT)
				...
				if techState != CIV_HAS_TECH:
					iProgress = self.CyTeam.getResearchProgress(iTech)
					if iProgress > 0 or techState == CIV_IS_RESEARCHING:
						barId = "progbar" + szTech
						screen.addStackedBarGFCAt(barId, szTechRecord, 1, self.hCell - 14, self.wCell - 2, 14, InfoBarTypes.NUM_INFOBAR_TYPES, eWidGen, 1, 2)
						screen.setStackedBarColorsRGB(barId, InfoBarTypes.INFOBAR_STORED, 255, 255, 255, 255)
						screen.setStackedBarColorsRGB(barId, InfoBarTypes.INFOBAR_RATE, 200, 200, 200, 128)
						screen.setStackedBarColorsRGB(barId, InfoBarTypes.INFOBAR_EMPTY, 0, 0, 0, 0)
						screen.setHitTest(barId, HitTestTypes.HITTEST_NOHIT)
						iCost = self.CyTeam.getResearchCost(iTech)
						iOverflow = self.CyPlayer.getOverflowResearch() * self.CyPlayer.calculateResearchModifier(iTech) / 100
	...
	def cacheBenefits(self):
		...
		while iTech < self.iNumTechs:
			...
			iTech += 1
		self.iVerticalCells = iMaxGridY / 2 + 1
	...
	def hideEraTable(self, screen):
		screen.hide("TC_EraTable")
		screen.hide("TC_EraTableBG")
		screen.hide("TC_EraTableBG0")
	...
	def redraw(self):
		...
		if self.updates:
			...
			self.screen().setForcedRedraw(False)

	def update(self, fDelta):
		self.screen().setFocus("TechChooser")
		if self.bLockedTT:
			POINT = Win32.getCursorPos()
	...
	def handleInput(self, inputClass):
		...
		if iCode == 4: # Mouse Enter
			...
			if NAME == "TC_JumpTo":
				screen.show("TC_EraTable")
				screen.show("TC_EraTableBG")
				screen.show("TC_EraTableBG0")
			elif NAME == "cycleTech":
				if ID:
					if self.currentSelection[1] == self.currentSelection[0] + 1:
						self.currentSelection[1] = 2
						self.scrollTo(self.currentSelection[self.currentSelection[1]])
					else:
						self.currentSelection[1] = self.currentSelection[1] + 1
						self.scrollTo(self.currentSelection[self.currentSelection[1]])
				else:
					if self.currentSelection[1] < 3:
						self.currentSelection[1] = self.currentSelection[0] + 1
						self.scrollTo(self.currentSelection[self.currentSelection[1]])
					else:
						self.currentSelection[1] = self.currentSelection[1] - 1
						self.scrollTo(self.currentSelection[self.currentSelection[1]])

			elif NAME == "scrollArrow":
				if ID:
					self.scrollTo(self.scrollOffs + self.xCellDist)
				else:
					self.scrollTo(self.scrollOffs - self.xCellDist)
				...
				if szFlag == "MOUSE_RBUTTONUP":
					if TYPE == "UNIT":
						...
						UP.pediaJumpToEra(self.iDisplayedEra)
					...
					if TYPE == "TECH":
						if self.CyPlayer.getAdvancedStartPoints() > -1:
							...
							elif CASE[0] == "CHOICE" and (self.currentTechState[iType] == CIV_TECH_AVAILABLE or not bShift and (self.currentTechState[iType] in (CIV_IS_RESEARCHING, CIV_IS_QUEUED, CIV_IS_TARGET))):
								CyMessageControl().sendResearch(iType, bShift)
								self.updateTechRecords(False)
						screen.setFocus("TC_Exit")
					elif TYPE == "ERA":
						self.scrollTo(self.minEraX[self.iDisplayedEra] - self.minX)
			...
			if NAME == "TC_DebugDD":
				...
				if self.currentSelection[0]:
					screen.show("cycleTech0")
					screen.show("cycleTech1")
				else:
					screen.hide("cycleTech0")
					screen.hide("cycleTech1")
				screen.addPanel(SCREEN_PANEL, "", "", False, False, 0, hPnlBarTop, self.maxX + self.xCellDist, self.yRes - hPnlBarTop - hPnlBarBot, PanelStyles.PANEL_STYLE_EMPTY)
				self.refresh(xrange(self.iNumTechs), False)
				self.initialScroll()
			elif NAME == "TC_JumpToEraDD":
				iEra = screen.getPullDownData("TC_JumpToEraDD", screen.getSelectedPullDownID("TC_JumpToEraDD"))
				self.scrollTo(self.minEraX[iEra] - self.minX)
			elif NAME == "TC_EraTable":
				self.hideEraTable(screen)
				self.scrollTo(self.minEraX[self.iNumEras - 1 - iData] - self.minX)

	def back(self):
		if self.iDisplayedEra:
			self.scrollTo(self.minEraX[self.iDisplayedEra - 1] - self.minX)

	def forward(self):
		if self.iDisplayedEra + 1 != self.iNumEras:
			self.scrollTo(self.minEraX[self.iDisplayedEra + 1] - self.minX)
	...
	def onClose(self):
		...
		del (
			self.InputData, self.szTxtTT, self.iOffsetTT, self.bLockedTT, self.iUnitTT, self.bUnitTT, self.iDisplayedEra,
			...
			self.eraTechBarred, self.eraTechAvailable, self.currentSelection
		)