...
void CvGame::assignStartingPlots(const bool bScenario, const bool bMapScript)
{
	...
	if (bScenario || bMapScript)
	{
		foreach_(CvPlot& plot, GC.getMap().plots() | filtered(bind(CvPlot::isStartingPlot, _1)))
		{
			...
			if (bScenario && algo::contains(alivePlayers | transformed(bind(CvPlayer::getStartingPlot, _1)), &plot))
			{
				continue;
			}
	...
	foreach_(CvPlayerAI* playerX, alivePlayers | filtered(bind(CvPlayer::getStartingPlot, _1) == nullptr))
	{
		...
		else alivePlayers.push_back(playerX);
	}
	...
	foreach_(CvPlot& plot, GC.getMap().plots())
	{
		if (plot.plotCount(PUF_isOtherPlayer, ePlayer) == 0)
		{
			...
			// Ensure that pPlot has the MapType of the unit... no need to include MapTypes on the Spawn Info file.
			{
				...
				{
					if (plot.isMapType((MapTypes)unitInfo.getMapType(iI)))
					{
						bFound = true;
					}
				}
