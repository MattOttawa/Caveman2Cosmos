...
{
	return algo::min_element(units() | transformed(bind(CvUnit::SMCargoVolume, _1))).get_value_or(MAX_INT);
}
...
{
	return algo::accumulate(units() | transformed(bind(CvUnit::baseCombatStr, _1)), 0);
}

int CvSelectionGroup::getStrength() const
{
	return algo::accumulate(
		units() | transformed(bind(CvUnit::AI_genericUnitValueTimes100, _1, UNITVALUE_FLAGS_DEFENSIVE | UNITVALUE_FLAGS_OFFENSIVE)), 0
	) / 100;
}
