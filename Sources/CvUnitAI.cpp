				...
				else
				{
					...
					if (pMissionPlot == pCitySitePlot && getGroup()->AI_getMissionAIType() == MISSIONAI_FOUND)
					{
						...
						if (getGroup()->canDefend() && getGroup()->getStrength() > (AI_minSettlerDefense() / 2))
						{
							...
							const CvPlot* pEndTurnPlot = getPathEndTurnPlot();
...
void CvUnitAI::AI_attackMove()
{
...

	{
		...
		else if (area()->getAreaAIType(getTeam()) != AREAAI_DEFENSIVE && area()->getCitiesPerPlayer(BARBARIAN_PLAYER) > 0
	...
	if (bReadyToAttack)
	{
		...
		if (iCityCaptureCount == 0 && !isCargo())
		{
			...
			if (gUnitLogLevel > 2)
			{
				CvWString szString;
				getUnitAIString(szString, AI_getUnitAIType());
				logBBAI("	%S's %S at (%d,%d) requests initial city-capture-capable attacker for attack stack at priority %d", GET_PLAYER(getOwner()).getCivilizationDescription(0), getName(0).GetCString(), getX(), getY(), HIGH_PRIORITY_ESCORT_PRIORITY + 1);
			}
		}
	}
	...
	for (int iPlotIndex = 0; iPlotIndex < NUM_CITY_PLOTS_2; iPlotIndex++)
	{
		...
		if (pPlot != NULL && pPlot->getOwner() == getOwner())
		{
			...
			if (pCity != NULL && plotSet.find(pPlot) != plotSet.end())
			{
				...
				int iPlotDanger2 = GET_PLAYER(getOwner()).AI_getPlotDanger(pPlot, 2);
...
bool CvUnitAI::AI_guardCitySite()
{
	...
	for (int iI = 0; iI < GET_PLAYER(getOwner()).AI_getNumCitySites(); iI++)
	{
		CvPlot* pLoopPlot = GET_PLAYER(getOwner()).AI_getCitySite(iI);
		if (GET_PLAYER(getOwner()).AI_plotTargetMissionAIs(pLoopPlot, MISSIONAI_GUARD_CITY, getGroup()) == 0)
		{
			int iPathTurns;
			if (generatePath(pLoopPlot, 0, true, &iPathTurns))
			{
				const int iValue = pLoopPlot->getFoundValue(getOwner());
				if (iValue > iBestValue)
				{
					iBestValue = iValue;
					pBestPlot = getPathEndTurnPlot();
...
bool CvUnitAI::AI_destroySpy()
{
	...
	for (iI = 0; iI < MAX_PC_PLAYERS; iI++)
	{
		...
		&& GET_PLAYER(getOwner()).AI_getAttitude((PlayerTypes)iI) <= ATTITUDE_ANNOYED)
		{
			foreach_(CvCity* pLoopCity, GET_PLAYER((PlayerTypes)iI).cities())
			{
				if (AI_plotValid(pLoopCity->plot()))
				{
					if (GET_PLAYER(getOwner()).AI_plotTargetMissionAIs(pLoopCity->plot(), MISSIONAI_ATTACK_SPY, getGroup()) == 0)
					{
	...
	for (iI = 0; iI < GC.getMap().numPlots(); iI++)
	{
		...
		if (AI_plotValid(pLoopPlot))
		{
			if (pLoopPlot->isOwned())
			{
				...
				{
					if (abPlayerAngry[pLoopPlot->getOwner()])
					{
						...
						if (eNonObsoleteBonus != NO_BONUS)
						{
							...
							if ((eImprovement != NO_IMPROVEMENT) && GC.getImprovementInfo(eImprovement).isImprovementBonusTrade(eNonObsoleteBonus))
							{
								...
								{
									iValue = GET_PLAYER(pLoopPlot->getOwner()).AI_bonusVal(eNonObsoleteBonus);

									if (pLoopPlot->isConnectedToCapital() && (pLoopPlot->getPlotGroupConnectedBonus(pLoopPlot->getOwner(), eNonObsoleteBonus) == 1))
									{
										iValue *= 3;
									}
									...
									if (iValue > 25)
									{
										if (GET_PLAYER(getOwner()).AI_plotTargetMissionAIs(pLoopPlot, MISSIONAI_ATTACK_SPY, getGroup()) == 0)
										{
...
bool CvUnitAI::AI_heal(int iDamagePercent, int iMaxPath)
{
	...
	if (bCanClaimTerritory || plot()->isCity() && plot()->getOwner() == getOwner())
	{
		...
		foreach_(CvUnit* pUnitToHeal, aeDamagedUnits)
		{
...
bool CvUnitAI::AI_spreadReligion()
{
	...
	|| algo::none_of_equal(*m_cachedMissionaryPlotset, plot()))
	{
		SAFE_DELETE(m_cachedMissionaryPlotset);
	...
	const CvPlot* pBestPlot = NULL;
	const CvPlot* pBestSpreadPlot = NULL;
	...
	if (getGroup()->AI_getMissionAIType() == MISSIONAI_SPREAD)
	{
		...
		const CvPlot* pPlot = getGroup()->AI_getMissionAIPlot();
		...
		if (pLoopCity != NULL
		&& algo::any_of_equal(*m_cachedMissionaryPlotset, pPlot)
		&& generatePath(pPlot, iBaseFlags, true))
		{
			...
			pBestPlot = bForceMove ? pPlot : getPathEndTurnPlot();
			...
			if (!bForceMove && owner.AI_getVisiblePlotDanger(pBestPlot, 1, false))
			{
				...
				if (generatePath(pPlot, MOVE_NO_ENEMY_TERRITORY, true))
				{
					//logBBAI("...found one");
					pBestPlot = getPathEndTurnPlot();
		...
		for (int iI = 0; iI < MAX_PC_PLAYERS; iI++)
		{
			...
			if (iPlayerMultiplierPercent > 0)
			{
				foreach_(CvCity * cityX, GET_PLAYER((PlayerTypes)iI).cities())
				{
					...
					&& algo::any_of_equal(*m_cachedMissionaryPlotset, cityX->plot())
			...
			if (generateSafePathforVulnerable(pLoopCity->plot(), &iPathTurns))
			{
				for (iI = 0; iI < GC.getNumSpecialistInfos(); iI++)
				{
					...
					if (canJoin(pLoopCity->plot(), ((SpecialistTypes)iI)))
					{
						...
						if (iOurThreat < (2 * iTotalThreat) / GET_PLAYER(getOwner()).getNumCities())
						{
			...
			for (CvReachablePlotSet::const_iterator itr = plotSet.begin(); itr != plotSet.end(); ++itr)
			{
					...
					)
				{
					...
					if (bAnimalDanger)
					{
						...
						else if (pLoopPlot->getOwner() != NO_PLAYER && !atWar(pLoopPlot->getTeam(), getTeam()))
						{
							iValue += 50;
						}
					}
					...
					if (iValue > iBestValue)
					{
						if (GET_PLAYER(getOwner()).AI_getAnyPlotDanger(pLoopPlot, 1))
						{
							iValue /= 2;
						}
						if (iValue > iBestValue)
						{
							...
							if (endTurnPlot == pLoopPlot ||
									!GET_PLAYER(getOwner()).AI_getAnyPlotDanger(endTurnPlot, 1) ||
		...
		if (/*AI_plotValid(pLoopPlot) &&*/ !pLoopPlot->isVisibleEnemyDefender(this) && pLoopPlot->area() == area() && (pLoopPlot->isRevealed(getTeam(), false) || pLoopPlot->isAdjacentRevealed(getTeam())))
		{
				...
				/*************************************************************************************************/
			{
				...
				foreach_(const CvPlot * pAdjacentPlot, pLoopPlot->adjacent())
				{
					...
					else if (bNoContact && pAdjacentPlot->getRevealedTeam(getTeam(), false) != pAdjacentPlot->getTeam())
					{
						iValue += 100;
					}
				}
...
bool CvUnitAI::AI_exploreRange(int iRange)
{
	...
	foreach_(const CvReachablePlotSet::const_iterator& itr, CvReachablePlotSet(getGroup(), MOVE_NO_ENEMY_TERRITORY, iSearchRange))
	{
		const CvPlot* pLoopPlot = itr.plot();

		//	Have we already considered this plot in a previous invocation?
		const int iOpaqueInfo = itr.getOpaqueInfo(ACTIVITY_ID_EXPLORE);
		...
		int iDX = abs(getX() - pLoopPlot->getX());
		int iDY = abs(getY() - pLoopPlot->getY());
	...
	int iBestValue = 0;
	const CvPlot* pBestPlot = NULL, *pBestExplorePlot = NULL;
	...
	while (plotValues.size() > 0)
	{
		...
		const int iValue = plotValues[plotValues.size() - 1].value;
		const CvPlot* pLoopPlot = plotValues[plotValues.size() - 1].plot;
...
bool CvUnitAI::AI_refreshExploreRange(int iRange, bool bIncludeVisibilityRefresh)
{
	...
	foreach_(const CvReachablePlotSet::const_iterator& itr, CvReachablePlotSet(getGroup(), MOVE_NO_ENEMY_TERRITORY, iSearchRange))
	{
		...
		if (pLoopPlot != NULL && !atPlot(pLoopPlot))
		{
			//if (AI_plotValid(pLoopPlot))
			{
				int iValue = 0 iAdjacentEnemies = 0;
			...
			if (GET_PLAYER((PlayerTypes)iI).isAlive() && ::isPotentialEnemy(getTeam(), GET_PLAYER((PlayerTypes)iI).getTeam()))
			{
				foreach_(CvCity * pLoopCity, GET_PLAYER((PlayerTypes)iI).cities())
				{
						...
						)
					{
						...
						/* TODO: cleanup this function - There is no target city, it was disabled
	...
	if (pBestCity == NULL)
	{
		...
		for (std::multimap<int, targetInfo>::const_iterator itr = possibleTargets.begin(); itr != possibleTargets.end(); ++itr)
		{
			...
			if (iBestValue > 0)
			{
				...
				const int iPathTurnsExtra = std::min(12, getGroup()->getBombardTurns(pLoopCity) / 4);
...
bool CvUnitAI::AI_goToTargetBarbCity(int iMaxPathTurns)
{
	...
	foreach_(const CvCity * pLoopCity, GET_PLAYER(BARBARIAN_PLAYER).cities())
	{
		if (AI_plotValid(pLoopCity->plot()))
		{
			...
			if (pLoopCity->isRevealed(getTeam(), false))
			{
				if (!atPlot(pLoopCity->plot()) && generatePath(pLoopCity->plot(), 0, true, &iPathTurns))
				{
					...
					{
						int iValue = GET_PLAYER(getOwner()).AI_targetCityValue(pLoopCity, false);
...
bool CvUnitAI::AI_anyAttack(int iRange, int iOddsThreshold, int iMinStack, bool bAllowCities, bool bFollow)
{
	...
	if (getDomainType() == DOMAIN_LAND && AI_claimForts(plotSet, iRange + 1))
	{
		return true;
	}
	...
	foreach_(const CvReachablePlotSet::const_iterator& itr, plotSet)
	{
		FAssert(itr.stepDistance() <= iSearchRange);
		//	Have we already considered this plot in a previous invocation?
		if (iMinStack == 0 && bAllowCities && !bFollow)
		{
			const int iOpaqueInfo = itr.getOpaqueInfo(ACTIVITY_ID_ANY_ATTACK);
	...
	for (int iI = 0; iI < MAX_PC_PLAYERS; iI++)
	{
		...
		foreach_(const CvCity * pCity, GET_PLAYER((PlayerTypes)iI).cities())
		{
			...
			foreach_(CvPlot * pLoopPlot, pCity->plot()->adjacent())
			{
				...
				&& canPlunder(pLoopPlot) && plotSet.find(pLoopPlot) != plotSet.end()
				&& GET_PLAYER(getOwner()).AI_plotTargetMissionAIs(pLoopPlot, MISSIONAI_BLOCKADE, getGroup(), 2) == 0)
				{
					CvReachablePlotSet::const_iterator itr = plotSet.find(pLoopPlot);
					...
					if (potentialWarAction(pLoopPlot))
					{
						...
						CvPlot* endTurnPlot;
						const int iValue = AI_blockadeValue(pLoopPlot, pCity, endTurnPlot);
...
bool CvUnitAI::AI_seaBombardRange(int iMaxRange)
{
	...
	foreach_(const CvReachablePlotSet::const_iterator& itr, CvReachablePlotSet(getGroup(), 0, iMaxRange))
	{
		const CvPlot* pLoopPlot = itr.plot();
	...
	if ((pBestPlot == NULL) && (pBestBombardPlot == NULL))
	{
		if (AI_getUnitAIType() != UNITAI_ASSAULT_SEA)
		{
			foreach_(const CvPlot * pLoopPlot, pPlot->rect(iMaxRange, iMaxRange))
			{
				if (AI_plotValid(pLoopPlot))
				{
					const CvCity* pBombardCity = bombardTarget(pLoopPlot);
...
/* Unit AI, Efficiency																		  */
/************************************************************************************************/
								//if( GET_PLAYER(pBombardCity->getOwner()).AI_getPlotDanger(pBombardCity->plot(), 2, false) > 0 )
	...
	if ((pBestPlot != NULL) && (pBestBombardPlot != NULL))
	{
		if (atPlot(pBestBombardPlot))
		{
			...
			if (bBombardUnitCanBombardNow && pGroup->canBombard(pBestBombardPlot))
			{
				...
				const CvCity* pBombardCity = bombardTarget(pBestBombardPlot); // is NULL if city cannot be bombarded any more
...
bool CvUnitAI::AI_pillage(int iBonusValueThreshold)
{
	...
	foreach_(const CvReachablePlotSet::const_iterator& itr, CvReachablePlotSet(getGroup(), 0, MAX_INT))
	{
		const CvPlot* pLoopPlot = itr.plot();
...
bool CvUnitAI::AI_pillageRange(int iRange, int iBonusValueThreshold)
{
	...
	foreach_(CvReachablePlotSet::const_iterator& itr, CvReachablePlotSet(getGroup(), 0, iSearchRange))
	{
		const CvPlot* pLoopPlot = itr.plot();

		//	Have we already considered this plot in a previous invocation?
		const int iOpaqueInfo = itr.getOpaqueInfo(ACTIVITY_ID_PILLAGE);
				...
				if (pWorkingCity != NULL)
				{
					if (pWorkingCity != area()->getTargetCity(getOwner()) && getGroup()->canPillage(pLoopPlot))
					{
						if (!pLoopPlot->isVisible(getTeam(), false) || !pLoopPlot->isVisibleEnemyUnit(this))
						{
							if (GET_PLAYER(getOwner()).AI_plotTargetMissionAIs(pLoopPlot, MISSIONAI_PILLAGE, getGroup()) == 0)
							{
								int iValue = 1000 * AI_pillageValue(pLoopPlot, iBonusValueThreshold);
...
bool CvUnitAI::AI_found()
{
	...
	for (int iI = 0; iI < GET_PLAYER(getOwner()).AI_getNumCitySites(); iI++)
	{
		...
		if ((pCitySitePlot->getArea() == getArea() || canMoveAllTerrain()) && canFound(pCitySitePlot)
		&& (!pCitySitePlot->isVisible(getTeam(), false) || !pCitySitePlot->isVisibleEnemyUnit(this))
		&& GET_PLAYER(getOwner()).AI_plotTargetMissionAIs(pCitySitePlot, MISSIONAI_FOUND, getGroup()) == 0
		&& getGroup()->canDefend() && getGroup()->getStrength() >= AI_minSettlerDefense()
...
bool CvUnitAI::AI_foundRange(int iRange, bool bFollow)
{
	...
	foreach_(CvPlot * pLoopPlot, plot()->rect(iSearchRange, iSearchRange))
	{
		if (AI_plotValid(pLoopPlot) && (pLoopPlot != plot() || GET_PLAYER(getOwner()).AI_getPlotDanger(pLoopPlot, 1) <= pLoopPlot->plotCount(PUF_canDefend, -1, -1, NULL, getOwner())))
		{
			if (canFound(pLoopPlot))
			{
				...
				if (iValue > iBestValue)
				{
					if (!pLoopPlot->isVisible(getTeam(), false) || !pLoopPlot->isVisibleEnemyUnit(this))
					{
						if (GET_PLAYER(getOwner()).AI_plotTargetMissionAIs(pLoopPlot, MISSIONAI_FOUND, getGroup(), 3) == 0)
						{
							if (generateSafePathforVulnerable(pLoopPlot, &iPathTurns))/*(generatePath(pLoopPlot, MOVE_SAFE_TERRITORY, true, &iPathTurns, iRange))*/
							{
								if (iPathTurns <= iRange)
								{
									const CvPlot* endTurnPlot = getPathEndTurnPlot();
...
bool CvUnitAI::AI_assaultSeaTransport(bool bBarbarian)
{
	...
	const bool bIsAttackCity = (getUnitAICargo(UNITAI_ATTACK_CITY) > 0);
	...
	const int iCargoCount = getGroup()->getCargo();
	int iBestValue = 0;
	const CvPlot* pBestPlot = NULL;
	const CvPlot* pBestAssaultPlot = NULL;

	foreach_(const CvReachablePlotSet::const_iterator& itr, CvReachablePlotSet(getGroup(), MOVE_AVOID_ENEMY_WEIGHT_3 | MOVE_ALLOW_ADJACENT_COASTAL, MAX_INT))
	{
		const CvPlot* pLoopPlot = itr.plot();
...
bool CvUnitAI::AI_specialSeaTransportMissionary()
{
	...
	foreach_(const CvReachablePlotSet::const_iterator& itr, CvReachablePlotSet(getGroup(), MOVE_ALLOW_ADJACENT_COASTAL, MAX_INT))
	{
		const CvPlot* pLoopPlot = itr.plot();
...
bool CvUnitAI::AI_specialSeaTransportSpy()
{
	...
	const CvPlot* pBestPlot = NULL;
	const CvPlot* pBestSpyPlot = NULL;
			...
			|| (!bSafeAutomation || eImprovement == eRuins) && !GC.getImprovementInfo(eImprovement).isCarriesIrrigation())
			{
				...
				&& pLoopPlot->isIrrigationAvailable(true))
				{
					...
					if (eBestTempBuild != NO_BUILD)
					{
						...
						if (bValid && (!pLoopPlot->isVisible(getTeam(), false) || !pLoopPlot->isVisibleEnemyUnit(this))
...
bool CvUnitAI::AI_fortTerritory(bool bCanal, bool bAirbase)
{
	...
	foreach_(const CvReachablePlotSet::const_iterator& itr, CvReachablePlotSet(getGroup(), isHuman() ? 0 : MOVE_IGNORE_DANGER, MAX_INT))
	{
		const CvPlot* pLoopPlot = itr.plot();
		...
		&& (pLoopPlot->getImprovementType() == NO_IMPROVEMENT || !pLoopPlot->isCityRadius()))
		{
			...
			if (iValue > iMinAcceptableValue)
			{
				...
				if (eBestTempBuild != NO_BUILD && (!pLoopPlot->isVisible(getTeam(), false) || !pLoopPlot->isVisibleEnemyUnit(this)))
				{
					bool bValid = true;
...
bool CvUnitAI::AI_retreatToCity(bool bPrimary, bool bAirlift, int iMaxPath)
{
	...
	{
		CvReachablePlotSet plotSet(getGroup(), (iPass > 1) ? MOVE_IGNORE_DANGER : 0, AI_searchRange(iMaxPath));
...
bool CvUnitAI::AI_pickupStranded(UnitAITypes eUnitAI, int iMaxPath)
{
	...
	int iBestValue = 0;
	const CvUnit* pBestUnit = NULL;
	const CvPlot* pBestPickupPlot = NULL;
	...
	foreach_(CvSelectionGroup * groupX, player.groups())
	{
		if (groupX->isStranded())
		{
			...
			const CvPlot* plotX = pHeadUnit->plot();
			...
			if (1000 * iCount > iBestValue)
			{
				...
				const CvPlot* pPickupPlot = NULL;
				if (atPlot(plotX) || algo::any_of_equal(plotSet, plotX))
				{
					pPickupPlot = plotX;
				}
				...
				else
				{
					foreach_(const CvPlot * plotY, plotX->adjacent())
					{
						...
						if (algo::any_of_equal(plotSet, plotY)
...
bool CvUnitAI::AI_airOffensiveCity()
{
	...
	for (int iI = 0; iI < GC.getMap().numPlots(); iI++)
	{
		...
		&& (atPlot(plotX) || canMoveInto(plotX)))
		{
...
bool CvUnitAI::AI_airDefensiveCity()
{
	...
	foreach_(CvCity * cityX, GET_PLAYER(getOwner()).cities())
	{
		...
		if (canAirDefend(plotX) && (atPlot(plotX) || canMoveInto(plotX)))
		{
			...
			if (GET_PLAYER((PlayerTypes)iI).isAliveAndTeam(getTeam(), false))
			{
				foreach_(const CvCity * cityX, GET_PLAYER((PlayerTypes)iI).cities())
				{
					if (isInvisible(cityX->getTeam(), false, true))
					{
						...
						int iValue = getEspionagePoints(cityX->plot());
						int iPathTurns;
...
bool CvUnitAI::AI_cityOffenseSpy(const int iMaxPath, const CvCity* pSkipCity)
{
	...
	const int iTreshold = (!bValid && GC.getGame().isOption(GAMEOPTION_AGGRESSIVE_AI) ? 51 : 1);
			...
			|| player.AI_getAttitudeWeight((PlayerTypes)iI) < iTreshold)
			{
				foreach_(const CvCity * cityX, playerX.cities())
				{
					...
					if (cityX->area() == pArea || canMoveAllTerrain())
					{
						...
						if (algo::any_of_equal(plotSet, plotX))
						{
...
bool CvUnitAI::AI_moveToStagingCity()
{
	...
	foreach_(CvCity * pLoopCity, GET_PLAYER(getOwner()).cities())
	{
		...
		if (pLoopCity->area() == area())
		{
			...
			if (itr != plotSet.end())
			{
				...
				//	is route-blind
				const int iMaxTurns = (iBestValue == 0 ? MAX_INT : 2 * iValue / iBestValue);
				...
				if (itr.stepDistance() <= iMaxTurns)
				{
					...
					const int iPriority = iValue / (5 + iPathTurns);
	...
	{
		const CvCity* pLoopCity = itr->second.first;
...
int CvUnitAI::AI_finalOddsThreshold(const CvPlot* pPlot, int iOddsThreshold) const
{
	...
	*/
	...
	if (iDefenders * 2 < getGroup()->getNumUnits() * 3)
	{
		...
		{
			if (GET_PLAYER(getOwner()).isModderOption(MODDEROPTION_AUTO_HUNT_ALLOW_UNIT_SUICIDING))
			{
		...
		{
			if (GET_PLAYER(getOwner()).isModderOption(MODDEROPTION_AUTO_PATROL_ALLOW_UNIT_SUICIDING))
			{
				int iInitialOddsThreshold = iFinalOddsThreshold;
				iFinalOddsThreshold /= getGroup()->getNumUnits();
				iFinalOddsThreshold *= 2;
	...
	else
	{
		...
		//int iPotentialAttackers = GET_PLAYER(getOwner()).AI_adjacentPotentialAttackers(pPlot, true);
...
bool CvUnitAI::AI_stackAttackCity(int iRange, int iPowerThreshold, bool bFollow)
{
	...
	foreach_(const CvPlot * pLoopPlot, plot()->rect(iSearchRange, iSearchRange))
	{
		...
		if (AI_plotValid(pLoopPlot)
			...
			&& (
				(bFollow && getGroup()->canMoveInto(pLoopPlot, true) || getGroup()->canMoveInto(pLoopPlot, false))
				||
...
bool CvUnitAI::AI_moveIntoNearestOwnedCity()
{
	...
	foreach_(const CvCity * pLoopCity, GET_PLAYER(getOwner()).cities())
	{
		if ((pLoopCity->area() == area()) && AI_plotValid(pLoopCity->plot()))
		{
			...
			&& (pLoopPlot->isCity() || pLoopPlot->isCity(true))
			&& canMoveInto(pLoopPlot) && generatePath(pLoopPlot, 0, true, &iPathTurns, 0))
			{
				FAssert(getPathEndTurnPlot() == pLoopPlot);
...
bool CvUnitAI::AI_artistCultureVictoryMove()
{
	...
	FAssertMsg(iCultureCitiesNeeded > 0, "AI_VICTORY_CULTURE1 Strategy should not be true");
...
bool CvUnitAI::AI_allowGroup(const CvUnit* pUnit, UnitAITypes eUnitAI) const
{
	...
	if (plot()->getOwner() == getOwner())
	{
		const CvPlot* pTargetPlot = pGroup->AI_getMissionAIPlot();
...
// Returns true if a mission was pushed...
bool CvUnitAI::AI_claimForts(const CvReachablePlotSet& kReachablePlots, int iMaxPath)
{
	PROFILE_FUNC();
	...
	foreach_(const CvReachablePlotSet::const_iterator& itr, kReachablePlots)
	{
		const CvPlot* pLoopPlot = itr.plot();
...
bool CvUnitAI::AI_StrategicForts()
{
	...
	CvPlot* pBestPlot = NULL;
	int iBestValue = 0;
	foreach_(CvPlot* pLoopPlot, CvReachablePlotSet(getGroup(), 0, MAX_INT)
	| transformed(bind(CvReachablePlotSet::const_iterator::plot, _1)))
	{
		if (/*AI_plotValid(pLoopPlot) &&*/ pLoopPlot->area() == area()
		...
		&& pLoopPlot->getImprovementType() == NO_IMPROVEMENT)
		{
			...
			if (eBestTempBuild != NO_BUILD
			&& (!pLoopPlot->isVisible(eTeam, false) || !pLoopPlot->isVisibleEnemyUnit(this))
	...
	for (iI = 0; iI < GC.getMap().numPlots(); iI++)
	{
		...
		if (AI_plotValid(pLoopPlot) && getGroup()->canPillage(pLoopPlot) && pLoopPlot->area() == area())
		{
			if (!pLoopPlot->isNPC() || !GET_PLAYER(getOwner()).isModderOption(MODDEROPTION_AUTO_PILLAGE_AVOID_BARBARIAN_CITIES))
			{
				if (potentialWarAction(pLoopPlot))
				{
					if (GET_PLAYER(getOwner()).isModderOption(MODDEROPTION_AUTO_PILLAGE_AVOID_ENEMY_UNITS) || !pLoopPlot->isVisible(getTeam(), false) || !pLoopPlot->isVisibleEnemyUnit(this))
					{
						if (GET_PLAYER(getOwner()).AI_plotTargetMissionAIs(pLoopPlot, MISSIONAI_PILLAGE, getGroup(), 1) == 0)
						{
							if (generatePath(pLoopPlot, 0, true, &iPathTurns))
...
void CvUnitAI::AI_borderPatrol()
{
	...
	bool bStayInBorders = !GET_PLAYER(getOwner()).isModderOption(MODDEROPTION_AUTO_PATROL_CAN_LEAVE_BORDERS);
	int iMinimumOdds = GET_PLAYER(getOwner()).getModderOption(MODDEROPTION_AUTO_PATROL_MIN_COMBAT_ODDS);
...
bool CvUnitAI::AI_InvestigatorFulfillment()
{
	...
	foreach_(const CvCity * pLoopCity, player.cities())
	{
		...
		iNumCriminals -= iResponders; //At this point iNumCriminals is more of a subjective need base
