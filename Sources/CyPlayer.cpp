...
{
	if (m_pPlayer) m_pPlayer->changeStabilityIndex(iChange);
}
...
{
	if (m_pPlayer) m_pPlayer->setColor((PlayerColorTypes)iColor);
}
/************************************************************************************************/
/* REVOLUTION_MOD                          END                                                  */
/************************************************************************************************/
...
{
	if (m_pPlayer) m_pPlayer->acquireCity(pCity->getCity(), bConquest, bTrade, true);
}

void CyPlayer::killCities()
{
	if (m_pPlayer) m_pPlayer->killCities();
}
...
{
	if (m_pPlayer) m_pPlayer->killUnits();
}
...
{
	return m_pPlayer ? m_pPlayer->getStateReligionName(iForm) : std::wstring();
}
...
{
	return m_pPlayer ? m_pPlayer->getArtStyleType() : -1;
}
...
{
	if (m_pPlayer) m_pPlayer->contact((PlayerTypes)ePlayer);
}
...
{
	if (m_pPlayer) m_pPlayer->receiveGoody(pPlot->getPlot(), (GoodyTypes) iIndex, pUnit->getUnit());
}

void CyPlayer::doGoody(CyPlot* pPlot, CyUnit* pUnit)
{
	if (m_pPlayer) m_pPlayer->doGoody(pPlot->getPlot(), pUnit->getUnit());
}
...
{
	if (m_pPlayer) m_pPlayer->found(x,y);
}
...
{
	if (m_pPlayer) m_pPlayer->setFreeUnitCountdown(iValue);
}
...
{
	return m_pPlayer ? m_pPlayer->getCurrentResearch() : NO_TECH;
}
...
{
	if (m_pPlayer) m_pPlayer->convert((ReligionTypes)iIndex);
}
...
{
	return m_pPlayer ? new CyPlot(m_pPlayer->getStartingPlot()) : NULL;
}

void CyPlayer::setStartingPlot(CyPlot* pPlot, bool bUpdateStartDist)
{
	if (m_pPlayer) m_pPlayer->setStartingPlot(pPlot ? pPlot->getPlot() : NULL, bUpdateStartDist);
}
...
{
	if (m_pPlayer) m_pPlayer->setGold(iNewValue);
}

void CyPlayer::changeGold(int iChange)
{
	if (m_pPlayer) m_pPlayer->changeGold(iChange);
}
...
{
	if (m_pPlayer) m_pPlayer->setEspionageSpendingWeightAgainstTeam((TeamTypes) eIndex, iValue);
}

void CyPlayer::changeEspionageSpendingWeightAgainstTeam(int /*TeamTypes*/ eIndex, int iChange)
{
	if (m_pPlayer) m_pPlayer->changeEspionageSpendingWeightAgainstTeam((TeamTypes) eIndex, iChange);
}
...
{
	if (m_pPlayer) m_pPlayer->changeGoldenAgeTurns(iChange);
}

void CyPlayer::changeNumUnitGoldenAges(int iChange)
{
	if (m_pPlayer) m_pPlayer->changeNumUnitGoldenAges(iChange);
}
...
{
	if (m_pPlayer) m_pPlayer->changeAnarchyTurns(iChange);
}
...
{
	return m_pPlayer ? m_pPlayer->getBestUnitType(UnitAITypes(eUnitAI)) : -1;
}
...
{
	if (m_pPlayer) m_pPlayer->changeExtraHealth(iChange);
}
...
{
	if (m_pPlayer) m_pPlayer->changeExtraHappiness(iChange, true);
}
...
{
	if (m_pPlayer) m_pPlayer->changeCoastalTradeRoutes(iChange);
}
...
{
	if (m_pPlayer) m_pPlayer->changeRevolutionTimer(addTime);
}
...
{
	if (m_pPlayer) m_pPlayer->changeStateReligionUnitProductionModifier(iChange);
}
...
{
	if (m_pPlayer) m_pPlayer->changeStateReligionBuildingProductionModifier(iChange);
}
...
{
	if (m_pPlayer) m_pPlayer->setFoundedFirstCity(bNewValue);
}

void CyPlayer::setAlive(bool bNewValue)
{
	if (m_pPlayer) m_pPlayer->setAlive(bNewValue);
}

void CyPlayer::setNewPlayerAlive(bool bNewValue)
{
	if (m_pPlayer) m_pPlayer->setNewPlayerAlive(bNewValue);
}

void CyPlayer::changeTechScore(int iChange)
{
	if (m_pPlayer) m_pPlayer->changeTechScore(iChange);
}
...
{
	return m_pPlayer ? m_pPlayer->getHandicapType() : -1;
}

int /*CivilizationTypes*/  CyPlayer::getCivilizationType() 
{
	return m_pPlayer ? m_pPlayer->getCivilizationType() : NO_CIVILIZATION;
}

int /*LeaderHeadTypes*/ CyPlayer::getLeaderType()
{
	return m_pPlayer ? m_pPlayer->getLeaderType() : -1;
}

int /*LeaderHeadTypes*/ CyPlayer::getPersonalityType()
{
	return m_pPlayer ? m_pPlayer->getPersonalityType() : -1;
}

void CyPlayer::setPersonalityType(int /*LeaderHeadTypes*/ eNewValue)
{
	if (m_pPlayer) m_pPlayer->setPersonalityType((LeaderHeadTypes) eNewValue);
}

int /*ErasTypes*/ CyPlayer::getCurrentEra()
{
	return m_pPlayer ? m_pPlayer->getCurrentEra() : NO_ERA;
}
...
{
	return m_pPlayer ? m_pPlayer->getStateReligion() : NO_RELIGION;
}

void CyPlayer::setLastStateReligion(int /*ReligionTypes*/ iNewReligion)
{
	if (m_pPlayer) m_pPlayer->setLastStateReligion((ReligionTypes) iNewReligion);
}
...
{
	return m_pPlayer ? m_pPlayer->getPlayerColor() : NO_COLOR;
}
...
{
	return m_pPlayer ? m_pPlayer->getSeaPlotYield(eIndex) : NO_YIELD;
}
...
{
	if (m_pPlayer) m_pPlayer->changeCommercePercent(eIndex, iChange);
}
...
{
	if (m_pPlayer) m_pPlayer->setOption((PlayerOptionTypes)eIndex, bNewValue);
}
...
{
	return m_pPlayer ? m_pPlayer->canHurry((HurryTypes)eIndex) : NO_HURRY;
}
...
{
	if (m_pPlayer) m_pPlayer->setCivics((CivicOptionTypes) eIndex, (CivicTypes) eNewValue);
}

int CyPlayer::getCombatExperience() const
{
	return m_pPlayer ? m_pPlayer->getCombatExperience() : -1;
}

void CyPlayer::changeCombatExperience(int iChange)
{
	if (m_pPlayer) m_pPlayer->changeCombatExperience(iChange);
}

void CyPlayer::setCombatExperience(int iExperience)
{
	if (m_pPlayer) m_pPlayer->setCombatExperience(iExperience);
}

int CyPlayer::getQueuePosition(int /*TechTypes*/ eTech)
{
	return m_pPlayer ? m_pPlayer->getQueuePosition((TechTypes)eTech) : -1;
}
...
{
	if (m_pPlayer) m_pPlayer->addCityName(szName);
}
...
void CyPlayer::trigger(int /*EventTriggerTypes*/ eEventTrigger)
{
	if (m_pPlayer) m_pPlayer->trigger((EventTriggerTypes)eEventTrigger);
}
...
{
	if (m_pPlayer) m_pPlayer->resetEventOccured((EventTypes)eEvent);
}
...
void CyPlayer::AI_updateFoundValues(bool bStartingLoc)
{
	...
	if (m_pPlayer) m_pPlayer->AI_updateFoundValues(true);
}
...
{
	if (m_pPlayer) m_pPlayer->AI_setAttitudeExtra((PlayerTypes)eIndex, iNewValue);
}

void CyPlayer::AI_changeAttitudeExtra(int /*PlayerTypes*/ eIndex, int iChange)
{
	if (m_pPlayer) m_pPlayer->AI_changeAttitudeExtra((PlayerTypes)eIndex, iChange);
}
...
{
	if (m_pPlayer) m_pPlayer->AI_changeMemoryCount((PlayerTypes)eIndex1, (MemoryTypes)eIndex2, iChange);
}
...
{
	return m_pPlayer ? m_pPlayer->getScoreHistory(iTurn) : 0;
}

int CyPlayer::getEconomyHistory(int iTurn) const
{
	return m_pPlayer ? m_pPlayer->getEconomyHistory(iTurn) : 0;
}

int CyPlayer::getIndustryHistory(int iTurn) const
{
	return m_pPlayer ? m_pPlayer->getIndustryHistory(iTurn) : 0;
}

int CyPlayer::getAgricultureHistory(int iTurn) const
{
	return m_pPlayer ? m_pPlayer->getAgricultureHistory(iTurn) : 0;
}

int CyPlayer::getPowerHistory(int iTurn) const
{
	return m_pPlayer ? m_pPlayer->getPowerHistory(iTurn) : 0;
}

int CyPlayer::getCultureHistory(int iTurn) const
{
	return m_pPlayer ? m_pPlayer->getCultureHistory(iTurn) : 0;
}

int CyPlayer::getEspionageHistory(int iTurn) const
{
	return m_pPlayer ? m_pPlayer->getEspionageHistory(iTurn) : 0;
}

int CyPlayer::getRevolutionStabilityHistory(int iTurn) const
{
	return m_pPlayer ? m_pPlayer->getRevolutionStabilityHistory(iTurn) : 0;
}
...
{
	if (m_pPlayer) m_pPlayer->setScriptData(szNewValue);
}

int CyPlayer::AI_maxGoldTrade(int iPlayer)
{
	CvPlayerAI* pPlayer = dynamic_cast<CvPlayerAI*>(m_pPlayer);
	return m_pPlayer ? pPlayer->AI_maxGoldTrade((PlayerTypes)iPlayer) : 0;
}

int CyPlayer::AI_maxGoldPerTurnTrade(int iPlayer)
{
	CvPlayerAI* pPlayer = dynamic_cast<CvPlayerAI*>(m_pPlayer);
	return m_pPlayer ? pPlayer->AI_maxGoldPerTurnTrade((PlayerTypes)iPlayer) : 0;
}

bool CyPlayer::canSplitEmpire() const
{
	return m_pPlayer ? m_pPlayer->canSplitEmpire() : false;
}

bool CyPlayer::canSplitArea(int iAreaId) const
{
	return m_pPlayer ? m_pPlayer->canSplitArea(iAreaId) : false;
}

bool CyPlayer::assimilatePlayer(int iPlayer)
{
	return m_pPlayer ? m_pPlayer->assimilatePlayer((PlayerTypes)iPlayer) : false;
}
...
{
	if (m_pPlayer) m_pPlayer->forcePeace((PlayerTypes)iPlayer);
}

void CyPlayer::addReminder(int iGameTurn, std::wstring szMessage) const
{
	if (m_pPlayer) m_pPlayer->addReminder(iGameTurn, CvWString(szMessage));
}
...
{
	if (m_pPlayer) m_pPlayer->setHandicap(iNewVal);
}

void CyPlayer::setModderOption(int /*ModderOptionTypes*/ eIndex, int iNewValue)
{
	if (m_pPlayer) m_pPlayer->setModderOption((ModderOptionTypes)eIndex, iNewValue);
}
...
{
	if (m_pPlayer) m_pPlayer->setAutomatedCanBuild((BuildTypes)eIndex, bNewValue);
}
...
{
	if (m_pPlayer) m_pPlayer->setCulture(iNewValue);
}

void CyPlayer::changeCulture(int iAddValue)
{
	if (m_pPlayer) m_pPlayer->changeCulture(iAddValue);
}

CvProperties* CyPlayer::getProperties()
{
	return m_pPlayer ? m_pPlayer->getProperties() : NULL;
}
...
{
	if (m_pPlayer) m_pPlayer->setBuildingListFilterActive((BuildingFilterTypes)eFilter, bActive);
}
...
{
	if (m_pPlayer) m_pPlayer->setBuildingListGrouping((BuildingGroupingTypes)eGrouping);
}
...
{
	if (m_pPlayer) m_pPlayer->setBuildingListSorting((BuildingSortTypes)eSorting);
}
...
{
	if (m_pPlayer) m_pPlayer->setUnitListInvalid();
}
...
{
	if (m_pPlayer) m_pPlayer->setUnitListFilterActive((UnitFilterTypes)eFilter, bActive);
}
...
{
	if (m_pPlayer) m_pPlayer->setUnitListGrouping((UnitGroupingTypes)eGrouping);
}
...
{
	if (m_pPlayer) m_pPlayer->setUnitListSorting((UnitSortTypes)eSorting);
}
...
{
	if (m_pPlayer) m_pPlayer->makeNukesValid(bValid);
}
...
{
	if (m_pPlayer) m_pPlayer->m_pBuildLists->writeToFile();
}
