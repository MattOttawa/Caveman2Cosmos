...
void CvCity::processSpecialist(SpecialistTypes eSpecialist, int iChange)
{
	...
	const UnitTypes eGreatPeopleUnit = (UnitTypes)kSpecialist.getGreatPeopleUnitType();
	...
	for (int iI = 0; iI < NUM_YIELD_TYPES; iI++)
	{
		changeExtraYield((YieldTypes)iI, (kSpecialist.getYieldChange(iI) + GET_PLAYER(getOwner()).getSpecialistYieldPercentChanges(eSpecialist, (YieldTypes)iI) / 100) * iChange);
	}
	...
	for (int iI = 0; iI < NUM_COMMERCE_TYPES; iI++)
	{
		...
		if (kSpecialist.getCommerceChange(iI) != 0)
		{
			iCommerceChangeTotal += (100 * kSpecialist.getCommerceChange(iI));
	...
	if (kSpecialist.getHealthPercent() > 0)
	{
		changeSpecialistGoodHealth(kSpecialist.getHealthPercent() * iChange);
	}
	else
	{
		changeSpecialistBadHealth(kSpecialist.getHealthPercent() * iChange);
	}
	if (kSpecialist.getHappinessPercent() > 0)
	{
		changeSpecialistHappiness(kSpecialist.getHappinessPercent() * iChange);
	}
	else
	{
		changeSpecialistUnhappiness(kSpecialist.getHappinessPercent() * iChange);
	}

	changeSpecialistFreeExperience(kSpecialist.getExperience() * iChange);

	changeSpecialistInsidiousness(kSpecialist.getInsidiousness() * iChange);
	changeSpecialistInvestigation(kSpecialist.getInvestigation() * iChange);

	foreach_(const UnitCombatModifier modifier, kSpecialist.getUnitCombatExperienceTypes())
	{
