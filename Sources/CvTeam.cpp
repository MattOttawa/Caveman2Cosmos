...
{
	if (eTeam == getID() || !isAlive() || !GET_TEAM(eTeam).isAlive() || isAtWar(eTeam) || !isHasMet(eTeam) || isForcePeace(eTeam))
	{
		return false;
	}
	...
	return canChangeWarPeace(eTeam, true);
}
				...
				if(GC.getGame().countKnownTechNumTeams(eIndex) == 1)
				{
					if (!Cy::call<bool>(PYGameModule, "doHolyCityTech", Cy::Args() << getID() << ePlayer << eIndex << bFirst))
					{
						if(!GC.getGame().isOption(GAMEOPTION_LIMITED_RELIGIONS))
						{
							...
							{
#endif
								if(GC.getGame().isTechCanFoundReligion(eIndex))
								{
									for (iI = 0; iI < GC.getNumReligionInfos(); iI++)
									{
										...
										if (eBestPlayer != NO_PLAYER)
										{
											...
											//GC.getGame().setReligionSlotTaken((ReligionTypes)iI, true);
/************************************************************************************************/
/* LIMITED_RELIGIONS               END                                                          */
/************************************************************************************************/
...
/************************************************************************************************/
			...
			if(GC.getGame().isTechCanFoundReligion(eIndex) && GC.getGame().isOption(GAMEOPTION_LIMITED_RELIGIONS)
#ifdef C2C_BUILD
				&& !GC.getGame().isOption(GAMEOPTION_DIVINE_PROPHETS))
#else
				)
#endif
			{
				if (!Cy::call<bool>(PYGameModule, "doHolyCityTech", Cy::Args() << getID() << ePlayer << eIndex << bFirst))
				{
					for (iI = 0; iI < GC.getNumReligionInfos(); iI++)
					{
						...
						eReligion = NO_RELIGION;
						eSlotReligion = ReligionTypes(iI);
						if ((GC.getReligionInfo(eSlotReligion).getTechPrereq() == eIndex) && !GC.getGame().isReligionSlotTaken(eSlotReligion))
						{
							for (iJ = 0; iJ < MAX_PLAYERS; iJ++)
							{
								CvPlayer& kPlayer = GET_PLAYER((PlayerTypes)iJ);
			...
			if (GC.getPromotionInfo((PromotionTypes)iI).getObsoleteTech() == eObsoleteTech)
			{
				for (int iPlayer = 0; iPlayer < MAX_PLAYERS; ++iPlayer)
				{
					...
					if (kLoopPlayer.isAlive() && GET_TEAM(kLoopPlayer.getTeam()).getID() == getID())
					{
						...
						{
							(*unitItr)->checkPromotionObsoletion();
						}
					}
				}
				break;
